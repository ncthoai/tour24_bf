...
class Contacts_Module_Model extends Vtiger_Module_Model {
	...
	public function getRelationQuery($recordId, $functionName, $relatedModule, $relationId) {
		if ($functionName === 'get_activities') {
            ...
            // Modified by Hieu Nguyen on 2019-11-21 to load related activities from vtiger_seactivityrel (link via parent_id)
			$query = "SELECT CASE WHEN (vtiger_users.user_name NOT LIKE '') THEN {$userNameSql} ELSE vtiger_groups.groupname END AS user_name, vtiger_seactivityrel.crmid AS parent_id,
                ...
                INNER JOIN vtiger_crmentity ON (vtiger_crmentity.crmid = vtiger_activity.activityid)
                LEFT JOIN vtiger_seactivityrel ON (vtiger_seactivityrel.activityid = vtiger_activity.activityid AND vtiger_seactivityrel.crmid = {$recordId})
                LEFT JOIN vtiger_invitees ON (vtiger_activity.activityid = vtiger_invitees.activityid AND vtiger_invitees.status = 'Accepted' AND vtiger_invitees.inviteeid = {$recordId})
                LEFT JOIN vtiger_users ON (vtiger_users.id = vtiger_crmentity.smownerid)
                LEFT JOIN vtiger_groups ON (vtiger_groups.groupid = vtiger_crmentity.smownerid)
                WHERE vtiger_crmentity.deleted = 0 AND vtiger_activity.activitytype <> 'Emails' AND (vtiger_seactivityrel.activityid IS NOT NULL OR vtiger_invitees.activityid IS NOT NULL)";
            // End Hieu Nguyen
